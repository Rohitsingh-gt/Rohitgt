...
class Database:
    ...
    def set_feedback_pending(self, user_id: int, pending: bool):
    self.users.update_one(
        {"user_id": user_id},
        {"$set": {"feedback_pending": pending}}
    )

def set_feedback_approved(self, user_id: int, approved: bool):
    self.users.update_one(
        {"user_id": user_id},
        {"$set": {"feedback_approved": approved}}
    )

def can_use_after_feedback(self, user_id: int) -> bool:
    user = self.get_user(user_id)
    if not user:
        return False
    return user.get("feedback_approved", False)
    ...
    def approve_user_with_feedback(self, user_id: int, days: int, feedback: str) -> bool:
    expires_at = get_current_time() + timedelta(days=days)
...
async def approve_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "❌ Usage: /approve <user_id> <days> <feedback>\n\n"
        "Example: /approve 123456789 30 Trusted User"
    )
    return
        ...
        # Approve user
        if db.approve_user_with_feedback(user_id, days, feedback):
            expires_at = get_current_time() + timedelta(days=days)
            await update.message.reply_text(
...
@admin_required
async def approvefeedback_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        if len(context.args) < 1:
            await update.message.reply_text(
                "❌ Usage:\n/approvefeedback user_id"
            )
            return

        user_id = int(context.args[0])

        db.set_feedback_pending(user_id, False)
        db.set_feedback_approved(user_id, True)

        await update.message.reply_text(
            f"✅ Feedback approved for {user_id}"
        )

        try:
            await context.bot.send_message(
                user_id,
                "✅ Your feedback has been approved by admin.\n"
        ...
        logger.error(f"Approve feedback error: {e}")
...
async def attack_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not db.can_use_after_feedback(user_id):
    await update.message.reply_text(
...
# NEXT USE LOCK
db.set_feedback_approved(user_id, False)
db.set_feedback_pending(user_id, True)
    ...
    try:
        ...
        if user.get("approved"):
            ...
            message = (
                ...
                f"📝 Admin Feedback: {user.get('admin_feedback', 'No feedback')}\n"
            )
        else:
            created_at_str = user.get('created_at').strftime('%Y-%m-%d') if user.get('created_at') else 'N/A'
...
async def feedback_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✅ Feedback submitted.\n"
        "⏳ Wait for admin approval before next use."
    )
...
def main():
 ...
 application.add_handler(CommandHandler("approvefeedback", approvefeedback_command))
